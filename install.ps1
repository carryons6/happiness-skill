#!/usr/bin/env pwsh
#
# Research Guardrail skill installer for Codex (Windows)
#
#   iwr -useb https://raw.githubusercontent.com/carryons6/research-guardrail-skill/main/install.ps1 | iex
#
# Installs the skill into  $env:CODEX_HOME\skills\research-guardrail  (default ~\.codex\skills\research-guardrail).
...
#   RESEARCH_GUARDRAIL_REPO_ZIP    zip URL used when git is unavailable
#
$ErrorActionPreference = 'Stop'

$SkillName = 'research-guardrail'
$Ref       = if ($env:RESEARCH_GUARDRAIL_REF)      { $env:RESEARCH_GUARDRAIL_REF }      else { 'main' }
$RepoUrl   = if ($env:RESEARCH_GUARDRAIL_REPO_URL) { $env:RESEARCH_GUARDRAIL_REPO_URL } else { 'https://github.com/carryons6/research-guardrail-skill.git' }
$RepoZip   = if ($env:RESEARCH_GUARDRAIL_REPO_ZIP) { $env:RESEARCH_GUARDRAIL_REPO_ZIP } else { "https://github.com/carryons6/research-guardrail-skill/archive/refs/heads/$Ref.zip" }
...
if ($git) {
  ...
  $tmp = Join-Path ([System.IO.Path]::GetTempPath()) ("research-guardrail-" + [System.Guid]::NewGuid().ToString())
  New-Item -ItemType Directory -Force -Path $tmp | Out-Null
  try {
    $zip = Join-Path $tmp 'research-guardrail.zip'
